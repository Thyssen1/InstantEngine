...
% Kernel_Yield_noise = 'ardrationalquadratic';
...
Sigma0_lb_Yield         = min(1e-2*std(lhs.Y(:,1)), gprMdlYield_Init.Sigma-1e-3);
Sigma0_lb_Yield_noise   = min(1e-2*std(lhs.Y_noise(:,1)), gprMdlYield_Init_noise.Sigma-1e-3);
...
Sigma0_lb_Tri         = min(1e-2*std(lhs.Y(:,2)), gprMdlTri_Init.Sigma-1e-3);
Sigma0_lb_Tri_noise   = min(1e-2*std(lhs.Y_noise(:,2)), gprMdlTri_Init_noise.Sigma-1e-3);
...
%         disp(strcat('(i,j)=(',num2str(i),',',num2str(j),')'))
        ...
        gprMdl_Yield_noise{i,j} = fitrgp(lhs.X(idxTrain,:), lhs.Y_noise(idxTrain,1),  ...
                        ...
                        'SigmaLowerBound', Sigma0_lb_Yield_noise, ...
                        'Standardize', true, ...
                        'ConstantSigma', true);
        ...
        gprMdl_Tri_noise{i,j} = fitrgp(lhs.X(idxTrain,:), lhs.Y_noise(idxTrain,2),    ...
                        ...
                        'SigmaLowerBound', Sigma0_lb_Tri_noise, ...
                        'Standardize', true, ...
                        'ConstantSigma', true);
        ...
        MSE_Yield(i,j)          = stats_Yield{i,j}.MSE;
        MSE_Yield_noise(i,j)    = stats_Yield_noise{i,j}.MSE;
        MSE_Tri(i,j)            = stats_Tri{i,j}.MSE;
        MSE_Tri_noise(i,j)      = stats_Tri_noise{i,j}.MSE;
        MSE_YieldVal(i,j)       = stats_YieldVal{i,j}.MSE;
        MSE_YieldVal_noise(i,j) = stats_YieldVal_noise{i,j}.MSE;
        MSE_TriVal(i,j)         = stats_TriVal{i,j}.MSE;
        MSE_TriVal_noise(i,j)   = stats_TriVal_noise{i,j}.MSE;
    end

    % Average over the seeds
    AverageYieldTrainLoss(i,1)       = mean(MSE_Yield(i,:));
    AverageYieldTrainLoss_noise(i,1) = mean(MSE_Yield_noise(i,:));
    AverageYieldValLoss(i,1)         = mean(MSE_YieldVal(i,:));
    AverageYieldValLoss_noise(i,1)   = mean(MSE_YieldVal_noise(i,:));

    AverageTriTrainLoss(i,1)       = mean(MSE_Tri(i,:));
    AverageTriTrainLoss_noise(i,1) = mean(MSE_Tri_noise(i,:));
    AverageTriValLoss(i,1)         = mean(MSE_TriVal(i,:));
    AverageTriValLoss_noise(i,1)   = mean(MSE_TriVal_noise(i,:));
...
ylabel('Loss (Mean squared error)', 'FontSize', 14)
